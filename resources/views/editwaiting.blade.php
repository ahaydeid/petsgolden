...
{{-- <p>Generated route: {{ route('pendingrequest.update', ['id' => $bookingDetail->id]) }}</p> --}}
                ...
                <form method="POST" action="{{ route('waitingrequest.update', ['id'=>$bookingDetail->id]) }}" style=" width: 90%; background-color: white; padding: 50px; box-shadow: 2px 2px 13px rgba(168,168,168,0.75);">
                    @csrf
                    @method('PUT')

                    <h1 class="text-center mb-5 fw-bold">REVIEW FOR PROCESSING</h1>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-8">
                            <button type="submit" name="action" value="ongoing" style="font-size:30px; width: 100%;" class="btn btn-success font-weight-bold py-1"><i class="fa fa-play"></i>&nbsp;Start Process</button>
                        </div>
